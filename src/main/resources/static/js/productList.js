window.addEventListener('DOMContentLoaded', function() {
    ...
    if (memberNo == productMemberNo) { // 로그인한 no와 상품 등록자의 no가 같으면
    ...
    function changeDealStatus() {
        ...
        if ( option == 'ing' ) { // 거래 중
            axios
            .get('/product/ing?productNo=' + productNo + '&boughtMemberNo=' + boughtMemberNo)
            .then(response => {
                isDealStatus()
            })
            .catch(err => { console.log(err); })
    ...
    function isDealStatus() { // 상품의 거래 상태 체크
        let str = '';
